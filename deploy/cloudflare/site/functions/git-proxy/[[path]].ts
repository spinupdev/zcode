/**
 * Cloudflare Pages Function: same-origin /git-proxy/* (stateless git CORS bridge).
 * Mirrors deploy/cloudflare/git-proxy Worker so the SPA default {origin}/git-proxy works.
 */
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  if (!matchesPrefix(url.pathname)) {
    return json({ error: 'not_found', hint: 'mount at /git-proxy/*' }, 404, request);
  }
