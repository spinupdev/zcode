#!/usr/bin/env bash
# Fetch marketplace theme/icon extensions used as ZCode workbench defaults.
#
...
# Theme/icon contributions only need package.json + assets.
...
extract_vsix() {
  ...
  rm -rf "${dest}"
  mkdir -p "${dest}"
  # Prefer rsync; fall back to cp for environments without it
  ...
  # Lightweight provenance for operators
...
# Sanity: required contribution paths
...
# Icon theme JSON path from published package.json
...
log "Theme extensions ready (icon: vscode-icons, color: GitHub light/dark)"
